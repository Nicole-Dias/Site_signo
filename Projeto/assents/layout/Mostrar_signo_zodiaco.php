<?php include('header.php'); $dt_nascimento = DateTime::createFromFormat('Y-m-d', $_POST['dt_nascimento']);
if(!$dt_nascimento){
...
function verificar_signo($data, $inicio, $fim){
    $ano= $data->format('Y');
    $data_inicio = DateTime::createFromFormat('d/n/Y', "$inicio/$ano");
    $data_fim = DateTime::createFromFormat('d/n/Y', "$fim/$ano");
...
foreach( $signos as $signo){
    if(verificar_signo($dt_nascimento,$signo->dataInicio, $signo->dataFim)){
